...
export const createUser = async (req, res) => {
    ...
    if (contentType === 'application/json') {
        ...
        try {
            ...
            res.status(201).json({ status: true, message: 'User created successfully.', data: userSave });
        } catch (error) {
            if (fileImage) {
                deleteOldImage(fileImage);
            }
            if (error.code === 11000) {
                return res.status(409).json({ status: false, message: 'Email may already be in use.' });
            }
...
export const getUsers = async (req, res) => {
    try {
        ...
        res.status(200).json({ status: true, message: 'User created successfully.', data: usersData});
    } catch (error) {
        res.status(500).json({ status: false, error: error.message });
    }
};
...
export const deleteUser = async (req, res) => {
    try {
        ...
        res.status(200).json({ status: true, message: 'User deleted successfully.', data: userDelete });
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: error.message });
    }
...
export const updateUser = async (req, res) => {
    ...
    if (contentType === 'application/json') {
        try {
            ...
            return res.status(200).json({ status: true, message: 'User updated successfully.', data: userUpdate });
        } catch (error) {
        ...
        upload.single("profileImage")(req, res, async (err) => {
            ...
            try {
                ...
                res.status(200).json({ status: true, message: 'User updated successfully.', data: userUpdate });
            } catch (error) {
                if (req.file) {
                    deleteOldImage(req.file.path);
                }
